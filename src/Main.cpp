...
arma::fmat g_EEt_sqrtEigenMat;

arma::fvec g_REEt_diagVec;
...
std::vector<std::string> g_permute_sample_ids;
				...
				bool t_is_permute_ginge)
{
	...
	// Reset permutation indices when new permutation parameters are set
	// This ensures clean state for each new analysis
	if(!g_is_permute_ginge){
		g_permute_ginge_indices.reset(); // Clear any previous custom indices
	}
	...
	if(g_is_permute_ginge){
		// Check if custom permutation indices have been set directly via R
		if(g_permute_ginge_indices.n_elem > 0){
			std::cout << "Using pre-set custom permutation indices" << std::endl;
			// Verify that the indices match the expected sample size
			if(g_permute_ginge_indices.n_elem != (ptr_gSAIGEobj->g_I_longl_mat).n_cols){
				std::cout << "WARNING: Custom permutation indices size (" << g_permute_ginge_indices.n_elem 
				          << ") does not match sample size (" << (ptr_gSAIGEobj->g_I_longl_mat).n_cols << ")" << std::endl;
			}
		} else if(!g_permute_sample_ids.empty()){
			std::cout << "Using custom permutation order from FAM file" << std::endl;
			// Custom permutation indices will be set later when we have access to sample IDs
			// For now, initialize with identity indices
			int n_samples = (ptr_gSAIGEobj->g_I_longl_mat).n_cols;
			g_permute_ginge_indices = arma::linspace<arma::uvec>(0, n_samples-1, n_samples);
		} else {
			std::cout << "Using random permutation for G×E" << std::endl;
			g_permute_ginge_indices = arma::randperm((ptr_gSAIGEobj->g_I_longl_mat).n_cols);  // Get random indices
		}
	}
...
//G x E
	...
	if(g_is_permute_ginge){
		arma::vec t_GVec_permute = t_GVec.elem(g_permute_ginge_indices);
        	if((ptr_gSAIGEobj->g_I_longl_mat.n_cols != ptr_gSAIGEobj->g_I_longl_mat.n_rows) && (t_GVec.n_elem < ptr_gSAIGEobj->m_y_gxe_mt.n_rows)){
                	t_GVec1 = (ptr_gSAIGEobj->g_I_longl_mat) * t_GVec_permute;
                	indexNonZeroVec_arma = arma::find(t_GVec1 > 0.0);
                	indexZeroVec_arma = arma::find(t_GVec1 == 0.0);
                	gtildeVec.set_size(t_GVec1.n_elem);
        	}else{
...
void set_EMat(arma::mat & t_EMat){
	...
	g_EEt_sqrtEigenMat.set_size(arma::size(g_EEt_eigenvecMat));
	for(unsigned int i = 0; i < g_EEt_eigenvalVec.n_elem; i++){
		arma::fcolvec sqrtEigenVec = std::sqrt(g_EEt_eigenvalVec(i)) * (g_EEt_eigenvecMat.col(i));
		g_EEt_sqrtEigenMat.col(i) = sqrtEigenVec;
	}

	g_REEt_diagVec.set_size(g_EMat.n_rows);
	g_REEt_diagVec.zeros();
        for(unsigned int i = 0; i < g_EEt_eigenvalVec.n_elem; i++){
                 arma::fvec eigenvec = g_EEt_eigenvecMat.col(i);
		 double eigenval = g_EEt_eigenvalVec(i);
		g_REEt_diagVec = g_REEt_diagVec + eigenval*arma::square(eigenvec);
	}
}
        ...
        if (use_base_optimization) {
            ...
            if (k_cols == Nnomissing) {
                ...
                Rcpp::stop("E matrix dimensions don't match expected. Raw: %d x %d, Expected: k x %d or %d x k", 
                          k_raw, k_cols, Nnomissing, Nnomissing);
            }
            ...
            bool use_individual_woodbury = (k < 100);  // reasonable number of covariates
            
            if (!use_global_woodbury && !use_individual_woodbury) {
                std::cout << "Warning: Both global and individual Woodbury infeasible (k=" 
                          << k << " too large). Implementing streaming rank-1 updates..." << std::endl;
                          
                // Strategy: Apply k rank-1 Woodbury updates sequentially
                // This uses O(N) memory per iteration instead of O(k*N)
                
                // Step 1: Compute base solution (no E matrix)
                arma::fvec x_current(Nnomissing);
                x_current.zeros();
                
                if (tauVec(1) == 0) {
                    x_current = (wVec / tauVec(0)) % bVec;
                } else {
                    auto n = g_I_start_indices.n_elem - 1;
                    for (size_t j = 0; j < n; j++) {
                        size_t start = g_I_start_indices[j];
                        ...
                        for (size_t idx = start; idx < end; idx++){
                            x_current(idx) = (wVec(idx) / tauVec(0)) * bVec(idx) -
                                           (tauVec(1) * ((wVec(idx) / tauVec(0)) * sum_delta_b)) / sum_S;
                        }
                    }
                }
                
                // Step 2: Apply rank-1 updates for each covariate
                std::cout << "Applying " << k << " rank-1 Woodbury updates..." << std::endl;
                
                for (int covar = 0; covar < k; covar++) {
                    if (covar % 10 == 0) {
                        std::cout << "Processing covariate " << (covar+1) << "/" << k << std::endl;
                    }
                    ...
                    for (size_t j = 0; j < n; j++) {
                        ...
                        // For this individual, use average E value
                        float e_avg = 0;
                        for (size_t idx = start; idx < end; idx++) {
                            e_avg += e_covar(idx);
                        }
                        ...
                        // Fill U and V for this individual
                        for (size_t idx = start; idx < end; idx++) {
                            u_vec(idx) = std::sqrt(tauVec(2)) * e_avg;
                            v_vec(idx) = std::sqrt(tauVec(2)) * e_avg;
                        }
                    }
                    
                    // Compute A^(-1) * u for this covariate
                    arma::fvec A_inv_u(Nnomissing);
                    if (tauVec(1) == 0) {
                        A_inv_u = (wVec / tauVec(0)) % u_vec;
                    } else {
                        for (size_t j = 0; j < n; j++) {
                            size_t start = g_I_start_indices[j];
                            size_t end = g_I_start_indices[j+1];
                            ...
                            for (size_t idx = start; idx < end; idx++){
                                sum_S += wVec(idx);
                                sum_delta_u += wVec(idx) * u_vec(idx);
                            }

                            sum_S = 1 + tauVec(1) * (sum_S / tauVec(0));
                            sum_delta_u /= tauVec(0);

                            for (size_t idx = start; idx < end; idx++){
                                A_inv_u(idx) = (wVec(idx) / tauVec(0)) * u_vec(idx) -
                                             (tauVec(1) * ((wVec(idx) / tauVec(0)) * sum_delta_u)) / sum_S;
                            }
                        }
                    }
                    
                    // Apply rank-1 Woodbury update: (A + u*v^T)^(-1) = A^(-1) - (A^(-1)*u*v^T*A^(-1))/(1 + v^T*A^(-1)*u)
                    float denominator = 1.0 + arma::dot(v_vec, A_inv_u);
                    
                    if (std::abs(denominator) > 1e-12) {  // Avoid division by zero
                        float v_dot_x = arma::dot(v_vec, x_current);
                        x_current = x_current - (v_dot_x / denominator) * A_inv_u;
                    }
                }
            ...
            // Case 3: Fallback to PCG iteration for edge cases
            // Examples: insufficient tau parameters, complex covariance structures not yet optimized
            arma::fvec rVec = bVec;
            arma::fvec r1Vec;
            ...
            if (iter >= maxiterPCG){
                Rcpp::stop("PCG did not converge within the maximum number of iterations (%d). Try increasing maxiterPCG or adjusting tolPCG.", maxiterPCG);
            }
...
void set_permute_ginge_indices(std::vector<std::string> sample_ids) {
	g_permute_sample_ids = sample_ids;
	std::cout << "Set custom permutation order with " << sample_ids.size() << " samples" << std::endl;
}

// [[Rcpp::export]]
void set_permute_ginge_indices_direct(arma::ivec indices) {
	g_permute_ginge_indices = arma::conv_to<arma::uvec>::from(indices);
	std::cout << "Set custom permutation indices directly with " << indices.n_elem << " samples" << std::endl;
}
...
arma::fvec getPCG1ofSigmaAndVector_noV(arma::fvec& wVec,  arma::fvec& tauVec, arma::fvec& bVec, int maxiterPCG, float tolPCG, bool LOCO){
    ...
    if(g_isStoreSigma){
        ...
        // Use optimized block structure solution instead of PCG iteration
        // This function handles the noV (no dynamic covariates) case
        // The covariance model is similar to the base case: σ = τ[0] * W^(-1) + τ[1] * Z^T * Z
        
        if (tauVec.n_elem < 2 || tauVec(1) == 0) {
            // Simple diagonal case: σ = τ[0] * W^(-1)
            xVec = (wVec / tauVec(0)) % bVec;
        } else {
...
arma::fvec getPCG1ofSigmaAndVector_V(arma::fvec& wVec,  float tauVal, float tauVal0, arma::fvec& bVec, int maxiterPCG, float tolPCG){
    ...
    if(g_isStoreSigma){
        ...
        // Use optimized block structure solution instead of PCG iteration
        // The covariance model is: σ = tauVal0 * W^(-1) + tauVal * Z^T * Z
        // This matches the base case from getPCG1ofSigmaAndVector_multiV
        
        if (tauVal == 0) {
            // Simple diagonal case: σ = tauVal0 * W^(-1)
            xVec = (wVec / tauVal0) % bVec;
        } else {
            // Block structure optimization using Sherman-Morrison-Woodbury formula
            auto n = g_I_start_indices.n_elem - 1;
            bool use_parallel = (g_omp_num_threads > 1) && (n > 100);
...
arma::fcolvec getCrossprod_multiV_eMat(arma::fcolvec& bVec, arma::fvec& wVec, arma::fvec& tauVec, bool LOCO){
		...
		//else{
                ...
                if(g_isGRM){
                  ...
                  //crossProdVec = tauVec(0)*(bVec % (1/wVec));
		  crossProd1 = getprodImatImattbVec(bVec); 
		 /*
		 //auto g_omp_num_threads = omp_get_num_threads();
		 omp_set_num_threads(g_omp_num_threads);
		 auto n = g_I_start_indices.n_elem - 1;
	crossProd1.zeros(bVec.n_elem);	
	#pragma omp parallel
	{
...
arma::fvec getCrossprodMatAndI_eMat_Imat(arma::fcolvec& bVec, bool LOCO){
	...
	arma::fcolvec bVec_1, IbVec, GRM_I_bvec, rossProdVecGRM_1, crossProdVecGRM, crossProdVecGRM_1, crossProdVec;
        crossProdVec.zeros(bVec.n_elem);
	omp_set_num_threads(g_omp_num_threads);
   //std::cout << "g_omp_num_threads " << g_omp_num_threads << std::endl;
   //arma::vec timeoutput1 = getTime();
      //printTime(timeoutput1, timeoutput2, "Unified_getOneMarker");

#pragma omp parallel
{
	// Create a temporary accumulator vector for each thread
	arma::fcolvec crossProdVec_temp = arma::zeros<arma::fcolvec>(crossProdVec.n_elem);

	#pragma omp for
        for(unsigned int i = 0; i < g_EEt_eigenvalVec.n_elem; i++){
		arma::fcolvec sqrtEigenVec = g_EEt_sqrtEigenMat.col(i);
		arma::fcolvec bVec_1 = sqrtEigenVec % bVec;	
        	arma::fcolvec IbVec = g_I_longl_mat_t * bVec_1;
        	// Cross-product calculation
        	arma::fcolvec crossProdVecGRM = g_I_longl_mat * IbVec;
 		arma::fcolvec temp = sqrtEigenVec % crossProdVecGRM;
        	// Accumulate into the private temporary vector
        	crossProdVec_temp += temp;
        }
	// After the parallel section, add the temporary vector into the main vector
    	
	#pragma omp critical
    	crossProdVec += crossProdVec_temp;
   ...
   //for(int j = 0; j < 10; j++){
   //     std::cout << "crossProdVec[j] " << crossProdVec[j] << std::endl;
   // 	}
        return(crossProdVec);
}
...
arma::fvec getDiagOfSigma_multiV_eMat(arma::fvec& wVec, arma::fvec& tauVec, bool LOCO){
        ...
        tauind = tauind + 1;
          ...
          if(g_isGRM && g_isSparseGRM){
                ...
                //         double eigenval = g_EEt_eigenvalVec(i);
                //         arma::fvec eigenvec = g_EEt_eigenvecMat.col(i);
                //         diagVec0 += eigenval * diagVecG_I % arma::square(eigenvec);
                //
		//}

               diagVec = diagVec + tauVec(tauind) * g_REEt_diagVec;
...
arma::fvec GetTrace_multiV_eMat(arma::fmat Sigma_iX, arma::fmat& Xmat, arma::fvec& wVec, arma::fvec& tauVec, arma::ivec & fixtauVec, arma::fmat& cov1,  int nrun, int maxiterPCG, float tolPCG, float traceCVcutoff, bool LOCO){
	...
	arma::fvec traVec = arma::mean(temp_mat_update, 0).t();
	traVec.print("traVec");
        temp_mat.clear();
        temp_mat_update.clear();
        return(traVec);
